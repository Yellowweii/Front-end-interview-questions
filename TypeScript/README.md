...
1、interface 主要用于定义对象的结构（属性、方法），也可以用来定义函数类型；type 可以定义任何类型：对象、联合类型、交叉类型、元组、基本类型别名等，功能更广。<br>
2、interface：用 extends 继承，可以多继承；type：用交叉类型 & 组合。<br>
3、interface 支持声明合并（多次声明会自动合并）。type 不能重复定义同名类型（会报错）。<br>
