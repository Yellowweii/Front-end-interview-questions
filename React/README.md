...
1、组件自身状态发生改变，React 会通过Object.is()做一次浅比较，如果值没变，就不重新渲染<br>
2、props 变化<br>
3、父组件重新渲染，优化方式：React.memo(Component)，做浅比较，如果 props 值没变，就不渲染<br>
4、context 变化<br>
5、强制更新forceUpdate()，只有类组件才有<br>

#### React Context API 的原理以及大量使用 Context API带来的一些问题？

React Context API 用于在组件树中跨层级共享数据，避免 props drilling。使用时先通过 createContext 创建上下文，然后通过 Provider 提供 value，子组件通过 useContext 消费数据。当 Provider 的 value 发生变化时，所有使用该 context 的组件都会重新渲染。在实际项目中需要注意通过 useMemo 优化 value，避免不必要的渲染。
大量使用 Context API 会带来一些问题，因为 Context 的更新是广播式的，只要 Provider 的 value 发生变化，所有消费该 Context 的组件都会重新渲染，无法做到按需更新。
